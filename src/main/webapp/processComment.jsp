					...
					<div align="center">
						...
						<%
							try{
								...
								String sql = "insert into comments(name,comment,prodid,ratings,catid,datetime) Values(?,?,?,?,?,current_timestamp)";
								...
								String ratings = request.getParameter("choiceOfRatings");
								String catid = request.getParameter("catid");
								String catname = request.getParameter("catname");
														
								if(name.equals("") || comment.equals("")){
									response.sendRedirect("indProductPage.jsp?prodid="+ prodid +"&catname="+ catname +"&catid="+ catid +"&msg=Name/Comment is blank");
								}else{
									
									pstmt.setString(1, name);
									pstmt.setString(2, comment);
									pstmt.setInt(3, Integer.parseInt(prodid));
									pstmt.setString(4, ratings);
									pstmt.setInt(5, Integer.parseInt(catid));
																		
									int rec = pstmt.executeUpdate();
									
									if(rec > 0){
										out.println("Your " + rec + " comment(s) has been successfully sent<br/><br/>");
									}else{
										out.println("Failed to send comment.");
									}
									
								}
								out.println("<form action='indProductPage.jsp?prodid="+ prodid +"&catname="+ catname +"&catid="+ catid +"' method='post'>");
